...
为 `StatisticsCards` 增加统一的量级格式化函数，通过 Ant Design `Statistic.formatter` 输出模糊值，并使用 `Tooltip` 承载带千分位的精确数值。
...
| R8 | 在 `StatisticsCards` 内统一格式化四项指标：0 保持为 0，1–999 显示 `<1k`，其余按一位小数向下取整为 `k+`/`w+` 并移除无效 `.0`；格式化结果由 `Tooltip` 包裹，标题显示含千分位和语义符号的精确值。 | `frontend/src/pages/GitStatistics/GitStatisticsList/components/StatisticsCards.tsx` | 运行前端生产构建；人工核对边界值、四张卡片模糊值及悬浮精确值。 | 已验证 |
...
- 量级格式使用向下取整而非四舍五入，确保 `+` 表达“至少达到该值”且不会夸大原始数据；本次不修改 API、状态结构或统计计算。
...
| 2026-09-18 | 增加提交活动汇总量级展示与精确值 Tooltip 方案，等待确认后实施；使用模型 GPT-5。 | R8 | 尚未修改产品代码，无 API、数据结构、权限或迁移影响。 |
| 2026-09-18 | 用户确认 R8 方案，开始实施。 | R8 | 允许修改汇总卡片显示逻辑。 |
| 2026-09-18 | 抽取 `StatisticCard` 与统一量级格式化逻辑，为四项汇总增加精确值 Tooltip；经 `code-simplifier` 复核后改用显式分支与原生 hover 容器。 | R8 | 仅改变 `/git-statistics` 汇总卡片展示，不改变原始数值、统计计算或接口。 |
| 2026-09-18 | 用户完成人工验证。 | R8 | 确认模糊量级、悬浮精确值及符号展示符合预期。 |
...
- `pnpm build:frontend`：通过，包含 TypeScript 检查和 Vite 生产构建；存在项目既有的 Browserslist 数据过期与大 chunk 警告，不影响本次构建。
- 用户已人工验证 R8：四项模糊量级、悬浮精确值及新增/删除符号符合预期。
