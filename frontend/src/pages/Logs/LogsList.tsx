...
const LogsList: React.FC = () => {
  ...
  const [serverLogsType, setServerLogsType] = React.useState<string | undefined>();
  const [serverLogsDateRange, setServerLogsDateRange] = React.useState<[Dayjs, Dayjs] | null>(null);
  ...
  const [requestLogsStatusCode, setRequestLogsStatusCode] = React.useState<number | undefined>();
  const [requestLogsDateRange, setRequestLogsDateRange] = React.useState<[Dayjs, Dayjs] | null>(null);
  ...
  const [taskLogsStatus, setTaskLogsStatus] = React.useState<string | undefined>();
  const [taskLogsDateRange, setTaskLogsDateRange] = React.useState<[Dayjs, Dayjs] | null>(null);

  // 加载服务器日志
  const loadServerLogs = React.useCallback(async (page = 1, pageSize = 20) => {
    setLoading(true);
    try {
      ...
      if (serverLogsDateRange && serverLogsDateRange[0] && serverLogsDateRange[1]) {
  ...
  }, [serverLogsDateRange, serverLogsType]);

  // 加载请求日志
  const loadRequestLogs = React.useCallback(async (page = 1, pageSize = 20) => {
    setLoading(true);
    try {
      const params: LogsQueryParams = {
        page,
      ...
      if (requestLogsDateRange && requestLogsDateRange[0] && requestLogsDateRange[1]) {
  ...
  }, [requestLogsDateRange, requestLogsStatusCode]);

  // 加载定时任务日志
  const loadTaskLogs = React.useCallback(async (page = 1, pageSize = 20) => {
    setLoading(true);
    try {
      const params: LogsQueryParams = {
        page,
  ...
  }, [taskLogsDateRange, taskLogsStatus]);
  ...
  const handleServerLogsSearch = () => {
    loadServerLogs(1, serverLogsPageSize);
  };

  const handleRequestLogsSearch = () => {
    loadRequestLogs(1, requestLogsPageSize);
  };

  const handleTaskLogsSearch = () => {
          ...
          <Card style={{ marginBottom: 16 }}>
            <Space>
              <RangePicker
                ...
                onChange={(dates) => setServerLogsDateRange(dates as [Dayjs, Dayjs] | null)}
                format="YYYY-MM-DD"
              />
              <Select
                placeholder="选择类型"
              ...
              <Button type="primary" icon={<SearchOutlined />} onClick={handleServerLogsSearch}>
                查询
              </Button>
              <Button icon={<ReloadOutlined />} onClick={() => loadServerLogs(serverLogsPage, serverLogsPageSize)}>
                刷新
              </Button>
            </Space>
          </Card>
          ...
          <Card style={{ marginBottom: 16 }}>
            <Space>
              <RangePicker
                ...
                onChange={(dates) => setRequestLogsDateRange(dates as [Dayjs, Dayjs] | null)}
                format="YYYY-MM-DD"
              />
              <Select
                placeholder="选择状态码"
              ...
              <Button type="primary" icon={<SearchOutlined />} onClick={handleRequestLogsSearch}>
                查询
              </Button>
              <Button icon={<ReloadOutlined />} onClick={() => loadRequestLogs(requestLogsPage, requestLogsPageSize)}>
                刷新
              </Button>
            </Space>
          </Card>
          ...
          <Card style={{ marginBottom: 16 }}>
            <Space>
              <RangePicker
                ...
                onChange={(dates) => setTaskLogsDateRange(dates as [Dayjs, Dayjs] | null)}
                format="YYYY-MM-DD"
              />
              <Select
                placeholder="选择状态"
              ...
              <Button type="primary" icon={<SearchOutlined />} onClick={handleTaskLogsSearch}>
                查询
              </Button>
              <Button icon={<ReloadOutlined />} onClick={() => loadTaskLogs(taskLogsPage, taskLogsPageSize)}>
                刷新
              </Button>
            </Space>
          </Card>
