...
const getNextExecutionTime = (cronExpression?: string): number | undefined => {
  ...
  try {
    return CronExpressionParser.parse(cronExpression, { tz: "Asia/Shanghai" })
      .next()
      .getTime()
  } catch {
    return undefined
...
const TasksList: React.FC = () => {
  ...
  } = useRequest(tasksApi.getTasks)
  const { data: repositories = [] } = useRequest(getRepositoriesConfig)
  const {
    data: runs = [],
    loading: runsLoading,
    refresh: refreshRuns,
  } = useRequest(() => scanRunsApi.list({ limit: 30 }), {
  ...
  const handleToggle = async (plan: ScanTask, enabled: boolean) => {
    try {
      await (enabled
        ? tasksApi.enableTask(plan.id)
        : tasksApi.disableTask(plan.id))
  ...
  const planColumns: ColumnsType<ScanTask> = [
    ...
    },
    {
      ...
      render: (_, plan) => {
        ...
        return nextTime ? (
          dayjs(nextTime).format("YYYY-MM-DD HH:mm")
        ) : (
          <Text type="danger">Cron 无效</Text>
        )
      },
    },
    {
      ...
      render: (_, plan) => {
        ...
        return (
          <Tag color={meta.color}>
            {meta.label} · +{run.insertedCommits}
          </Tag>
        )
      },
    },
  ...
  const runColumns: ColumnsType<ScanRun> = [
    ...
    },
    {
      ...
      width: 110,
      render: (source: ScanRun["triggerSource"], run) =>
        source === "scheduled"
          ? "定时触发"
          : run.planId
            ? "计划手动执行"
    ...
    },
    {
      ...
      width: 220,
      render: (_, run) =>
        `${dayjs(run.rangeStart).format("YYYY-MM-DD")} 至 ${dayjs(run.rangeEnd).format("YYYY-MM-DD")}`,
    },
    {
      title: "数据源",
      dataIndex: "requestedRepositoryIds",
      width: 90,
      render: (ids: string[]) => `${ids.length} 个`,
    },
    {
      title: "新增",
      dataIndex: "insertedCommits",
      width: 90,
    ...
    },
    {
      ...
      width: 100,
      render: (_, record) => {
        return <Button type='link' className='px-0' onClick={() => setSelectedRun(record)}>详情</Button>
      },
    },
  ]
    ...
    <div className="p-3">
      ...
      >
        <div>
          ...
          <Text type="secondary">
            管理自动同步时间、扫描范围与数据源，所有时间按中国上海时区执行。
          </Text>
        </div>
        <Space>
          <Tooltip title="刷新">
