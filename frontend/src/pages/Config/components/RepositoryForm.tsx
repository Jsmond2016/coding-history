...
const RepositoryForm: React.FC<RepositoryFormProps> = ({
  ...
  const handleDiscover = async () => {
    try {
      ...
      setDiscovering(true)
      const result = await discoverAuthorsByPath(
      ...
      setSelectedEmails(
        result.candidates.slice(0, 1).map((candidate) => candidate.email),
      )
      setStep(1)
    } catch (error: any) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const selectedCandidates = candidates.filter((candidate) =>
        selectedEmails.includes(candidate.email),
      )
      const manualAuthor = values.manualAuthorEmail?.trim()
        ? {
            name:
              values.manualAuthorName?.trim() ||
              values.manualAuthorEmail.split("@")[0],
            email: values.manualAuthorEmail.trim(),
          }
        : undefined
    ...
    >
      {!repository ? (
        <div className="mt-[16px]">
          <Steps
            current={step}
            size="small"
            items={[{ title: "仓库信息" }, { title: "作者与首次扫描" }]}
          />
          <Divider />
        </div>
      ) : null}
      <Form
        ...
        onValuesChange={(changedValues) => {
          if (
            repository ||
            !Object.prototype.hasOwnProperty.call(changedValues, "path")
          )
            return
          ...
          if (!currentName || currentName === previous.name)
      ...
      >
        ...
        {!repository && step === 1 ? (
          <div>
            ...
            <Typography.Text type="secondary" className="block mb-2 mt-4">
              候选中没有需要的作者时，可手动补充：
            </Typography.Text>
            <Space.Compact className="w-full">
              <Form.Item name="manualAuthorName" noStyle>
                <Input placeholder="作者姓名" />
              </Form.Item>
              <Form.Item
                name="manualAuthorEmail"
                noStyle
