...
**需求描述**：
- 任务列表支持用户自定义排序
- 排序结果持久化到数据库，刷新页面后保持用户调整的顺序
- 提供"恢复默认排序"功能

#### 2.9.2 默认排序规则
**排序逻辑**（当 `sortOrder` 相同时按以下规则排序）：
1. **定时任务在前，手动任务在后**：按 `taskType` 排序，`scheduled` 优先于 `manual`
2. **定时任务按下次执行时间排序**：根据 `cronExpression` 计算下次执行时间，越早执行的排在越前面
3. **手动任务按最后执行时间排序**：最近执行的排在越前面，未执行过的排在后面
...
**交互要求**：
- 需要拖拽 5px 以上才开始拖拽（避免误触）
- 拖拽过程中当前行背景变浅灰（`#fafafa`）
- 拖拽完成后显示"排序已保存"提示
...
#### 2.9.5 数据模型
**数据库字段**：
- `scan_tasks.sort_order`：整型，默认值 0，数值越小越靠前
...
**批量更新排序**
- **接口路径**：`POST /api/v1/tasks/batch-sort`
- **请求体**：
```typescript
{
...
**获取任务列表**
- 返回的任务按 `sortOrder` 升序排序
- `sortOrder` 相同时按 `createdAt` 降序排序
